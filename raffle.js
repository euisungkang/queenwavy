...
async function initializeRaffle(channel, logs, winnerChannel, sendMessage) {
  ...
  const filter = (reaction, user) =>
    reaction.emoji.id == "814968693981184030" && user.id != msg.author.id;
...
async function calculateCurrency(raffle, channel, message, user, logs) {
  ...
  if (isNaN(message.first().content) && message.first().content != "all") {
    ...
    buffer = await channel.send(
      "You don't have enough <:HentaiCoin:814968693981184030>. Broke ass bitch"
    );
    await wait(3000);
    buffer.delete();
    return false;
  ...
  if (amount > max) {
    buffer = await channel.send(
      "Please check again how many <:HentaiCoin:814968693981184030> you can purchase"
    );
    await wait(3000);
    buffer.delete();
    return false;
...
async function alertCandidates(sendRaffleAlert) {
  ...
  let embed = await new Discord.MessageEmbed()
    ...
    .setThumbnail(
      "https://cdn.discordapp.com/attachments/824106380222005288/824110141305651240/artworks-000505954353-sqeh0j-t500x500.jpg"
    )
    .setDescription(
      "The raffle for **" +
        name +
